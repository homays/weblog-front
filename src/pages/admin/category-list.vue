    ...
    <div>
        ...
        <FormDialog ref="formDialogRef" title="添加文章分类" destroyOnClose @submit="onSubmit">
            <el-form ref="formRef" :rules="rules" :model="form">
                <el-form-item label="分类名称" prop="tagName" label-width="80px" size="large">
        ...
        <!-- 编辑分类 -->
        <FormDialog ref="editFormDialogRef" title="编辑标签" destroyOnClose @submit="editCategorySubmit">
            <el-form ref="editFormRef" :rules="rules" :model="editForm">
                <el-form-item label="标签名称" prop="name" label-width="80px" size="large">
                    <el-input v-model="editForm.name" placeholder="请输入分类名称" maxlength="20" show-word-limit clearable />
                </el-form-item>
            </el-form>
        </FormDialog>
...
import { getCategoryPageList, addCategory, editCategory, deleteCategory } from '@/api/admin/category'
...
const editCategorySubmit = () => {
  ...
  editFormRef.value.validate((valid) => {
    ...
    editCategory(editForm).then((res) => {
      if (res.success == true) {
        showMessage('添加成功')
        // 隐藏对话框
